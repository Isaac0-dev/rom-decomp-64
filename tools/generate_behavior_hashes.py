    ...
    for m in matches:
        ...
        if func_names_list and any(fn is not None for fn in func_names_list):
            for ci in range(len(func_names_list)):
                ...
                add_hash(h_variant, name)

    print("# This file is auto-generated by tools/generate_behavior_hashes.py")
    print("# Do not edit manually.")
    print("")
    print("from typing import Dict, List, Union")
    print("")
    print("KNOWN_BEHAVIOR_HASHES: Dict[str, Union[str, List[str]]] = {")
    for h, names in sorted(hashes.items(), key=lambda x: (x[1][0], x[0])):
        if len(names) == 1:
            print(f'    "{h}": "{names[0]}",')
        else:
